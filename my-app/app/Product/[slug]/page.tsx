...
//   const pathname = usePathname();
 const pathname = usePathname()
 const productList: unitType[] = conveyorUnits;


 const [product, setProduct] = useState<any | null>(null);
 const [productDetails, setProductDetails] = useState<unitType | null>(null);
  
 useEffect(() => {
    console.log('Product:', pathname.split('/').pop());
    setProduct(pathname.split('/').pop());
  ...
  }, [pathname]); // Dependencies for useMemo
                ...
                <div className="lg:max-w-6xl max-w-xl mx-auto">
                    <div className="grid items-start grid-cols-1 lg:grid-cols-2 gap-8 max-lg:gap-12 max-sm:gap-8">
                        ...
                        <div className="w-full">
                            <div>
                                <h3 className="text-lg sm:text-xl font-semibold text-slate-900">{filteredData[0]?.ProductName}</h3>
                                <p className="text-slate-500 mt-2 text-sm">Women Embroidered Georgette A-line Kurta With Attached Dupatta (Maroon)
                                </p>
                                <div className="flex items-center flex-wrap gap-4 mt-6">
                                    <h4 className="text-slate-900 text-2xl sm:text-3xl font-semibold">$12</h4>
                                    <p className="text-slate-500 text-lg"> <span className="text-sm ml-1.5">Tax included</span></p>
                                </div>
                            ...
                            <div>
                                <h3 className="text-lg sm:text-xl font-semibold text-slate-900">Sizes In Meeter</h3>
                                <div className="flex flex-wrap gap-4 mt-4">
                                    <button type="button"
                                        className="w-10 h-9 border border-slate-300 hover:border-blue-600 text-sm cursor-pointer flex items-center justify-center shrink-0">50</button>
                                    <button type="button"
                                        className="w-10 h-9 border border-blue-600 hover:border-blue-600 text-sm cursor-pointer flex items-center justify-center shrink-0">100</button>
                                    <button type="button"
                                        className="w-10 h-9 border border-slate-300 hover:border-blue-600 text-sm cursor-pointer flex items-center justify-center shrink-0">200</button>
                                    <button type="button"
                                        className="w-10 h-9 border border-slate-300 hover:border-blue-600 text-sm cursor-pointer flex items-center justify-center shrink-0">300</button>
                                </div>

                                <div className="mt-6 flex flex-wrap gap-4">
                                    <button type="button"
                                        className="px-4 py-3 w-[45%] cursor-pointer border border-slate-300 bg-slate-100 hover:bg-slate-200 text-slate-900 text-sm font-medium">Add
                                        to wishlist</button>
                                    <button type="button"
                                        className="px-4 py-3 w-[45%] cursor-pointer border border-blue-600 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium">Add
                                        to cart</button>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2 mt-6 max-w-sm">
                                    ...
                                    <button type='button'
                                        className="border-0 outline-0 bg-blue-600 hover:bg-blue-700 text-white cursor-pointer px-4 py-2.5 text-sm">Apply</button>
                                </div>
                            </div>
